...
- Classic 橘猫作为可选桌面陪伴回归；默认仍为 Mini，二者严格互斥。
- 增加 working、awake_rest、sleeping 三种基础状态、状态化单击和 500ms 长按拖拽。
- 增加逐帧时长、真实完成事件、左右方向切换、透明像素命中和故障回落。
- Workbench 打开期间隐藏当前陪伴，关闭或失败后只恢复进入前模式。

### 边界

- 首次回归只包含 Classic，不包含多多、多宠物、业务事件动作、下载系统或宠物市场。
- Classic 运行时素材采用受限产品许可，不适用 MIT；生产与 QA 中间文件不进入产品包。
- 本节仅记录本地候选事实，不代表 tag、GitHub Release 或公开发布已经完成。
...
- 根据真实业务边界显示距离上班、休息、恢复工作或下班，不再使用固定倒计时文案。
- 将今日安排收敛为时间、轴线、内容三列结构，并统一使用“休息”语义。
- 移除迷你视图冗余拖动横线，支持非交互区域全窗口拖动和状态化尺寸。
- 建立今天、选中日期、工作/休息类型、手动调整、焦点和过期数据的复合日历状态。
- 使用固定版本 Lucide React 统一今日、日历、设置、月份切换、关闭和重试图标。
- 明确调休工作日来源及跨夜班次归属日期文案。
- 增加浅色默认与深色双主题，支持本地持久化、预览回滚和跨窗口即时同步。
- 修复 WebView2 冷启动瞬时失败、二级窗口显示阻塞和复用 Wizard 首次配置语义错误。

### 已知边界

- 原生托盘继续使用 Windows 系统样式，不跟随 Web 主题。
- Windows 睡眠恢复、手动修改系统时间/时区和连续两小时运行保留为非阻塞环境补证。

## v1.0.1 Stable（2026-07-27）

### 状态

- 实现、独立验收和发布收口均已通过。
- 已通过 `v1.0.1` tag 发布 Windows x86_64 便携 Zip 与 `SHA256SUMS.txt`。
- 发布 Zip SHA256：`DB45332F908669445B34FF40C490936B0EEAC0B41DC2FCDC2F5806924E5D1AC2`。

### 修复与改进

- 接入 2025、2026 年中国大陆法定节假日与调休工作日离线数据。
- 日期调整支持工作日、带薪休息、不带薪休息和恢复自动，并完成失败补偿与持久化。
- 修正跨夜班次归属、真实秒级收益和 30 秒权威同步。
- 使用整数分累计差分配，消除完整工作月的分币尾差。
- 修正日期调整成功反馈被刷新吞掉的问题。
- 修正更新检查将旧版本误判为新版本的问题。
- 增加 Rust、TypeScript、配置事务和包内资源行为验证。

### 已知边界

- 官方日历当前覆盖 2025、2026，其他年份明确显示不支持。
- 睡眠恢复、系统时间/时区变化和连续两小时运行仍待人工补证。

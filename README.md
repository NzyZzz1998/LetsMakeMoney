...
当前已发布版本为 **v0.6 Beta**；`main` 正在公开开发 **v0.7 Beta**。v0.7 尚未完成 Acceptance、打包或发布，不能把源码进度当作可发布版本。最新事实源请先阅读 `doc/current.md` 和 `doc/releases/v0.7/current.md`。
...
v0.6 最终验收结论为“通过”，当前无发布阻塞项。真实 Windows 登录后的开机自启暂不验证，不得写为通过；该能力默认关闭，作为 Beta 已知限制和发布后观察项，不阻塞本次发布。v0.5 文档与发布包继续作为回滚基线。

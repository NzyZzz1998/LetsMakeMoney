...
- **测试候选已完成本机验收：** 动作体量连续性、拖拽手感、快速反转、窗口协同、托盘退出和三档 DPI 均已通过，等待最终发布候选重建。
...
> [!IMPORTANT]
> v1.1.0 当前只同步到测试分支，不是公开稳定版。Classic 的自动门禁与本机直接输入验收已通过，正式发布前仍需从最终干净提交重建并锁定新哈希。
...
| 人工结论 | 通过；本机完成拖拽、窗口协同、托盘退出和三档 DPI 闭环 |
...
- 正式发布前从最终文档提交后的干净源重新构建候选，并更新 Zip、EXE 与 DLL 哈希。
- 对最终重建包执行身份、包体和启动冒烟；无需无理由重复当前候选已经完成的完整 GUI 验收。

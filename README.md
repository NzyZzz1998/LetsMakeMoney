...
> [!IMPORTANT]
> v1.1.0 当前只同步到测试分支用于继续验收，不是公开稳定版。Classic 的体量连续性已改善，但拖拽手感仍需在本机直接输入环境确认。
...
| 当前干净候选 | `V110-20260813T081246Z-f5ae4ac3-clean` |
| 候选 Zip SHA256 | `E79D3716400D74E9E2F5419700B97630F273AA67761982081AED07E8C87C6EB7` |
| 自动门禁 | current gate、91 项 Rust 测试、Clippy、前端构建、包体验证通过 |
| 人工结论 | 部分通过；远程环境只确认体量改善，不能替代本机拖拽验收 |
...
- 本机直接输入下的抓取连续性、拖拽跟手性、快速左右反转，以及截图或失焦中断后的再次交互。
- 托盘右键命令逐项执行、退出后的进程状态，以及 Settings / Modal 的输入锁定与异常关闭恢复。
- 125% / 150% DPI 下的完整长按拖拽闭环；当前只证明界面、动画可见性和透明命中。
- 正式 EXE 内嵌坏包的受控桌面注入；自动隔离坏包夹具 3/3 已通过。
- Windows 10 与多显示器环境。
...
- **v1.1.0 候选：** 三档 DPI 的界面与透明命中通过；本机完整拖拽链路仍待补证。

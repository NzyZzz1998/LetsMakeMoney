...
1. V07-A 完成并经项目所有者批准后允许源码仓库先公开；V07-B 至 V07-E 和独立 Acceptance 继续约束 v0.7 构建承诺、安装器、更新、tag 与 Release。
...
### V07-D1：iOS/macOS/Android 平台规划

- 目标：以 iOS 为最高优先级，再依次说明 macOS、Android 的窗口/托盘/穿透、签名/商店、配置目录、触控与 Godot native 阻塞。
...
- 源码仓库公开门禁：A0-A3 完成、历史/资产/隐私无 P0，并由项目所有者批准。该门禁已关闭。
- v0.7 发布门禁：B-E 全部完成、构建可复现、Main/native 回归通过、社区入口可用并通过独立 Acceptance。
...
| 全量范围延期 | A-E 未全部完成 | 保持仓库公开开发，但不得提前发布 v0.7 tag、安装器、Zip 或更新能力 |
...
开发阶段完成意味着：A-E checklist 全部实现、自动测试通过、安装器/Zip 候选产物形成、公开 readiness 文档完整，并已进入独立 `/acceptance`。源码仓库可以公开协作，但开发阶段无权：

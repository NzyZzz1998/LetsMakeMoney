...
> 本文是 v0.7 的需求事实源。A0-A3 完成后，项目所有者新增决策：允许源码仓库先公开，B-E 继续作为 v0.7 工程、分发与发布门禁。仓库公开不代表业务代码已经实现，也不代表 v0.7 已经发布。
...
- 继续采用 A-E 全量方案；A0-A3 关闭安全公开门禁后先公开源码仓库，其余模块完成后再发布 v0.7 产物。
...
- iOS、macOS、Android、主题系统和更多宠物仅输出规划文档；平台规划优先级为 iOS、macOS、Android。
...
- 源码公开检查清单无未关闭 P0；v0.7 tag、安装器、Zip 和 Release 仍等待 B-E 与独立验收。
...
2. 平台规划按 iOS、macOS、Android 的优先级分析：窗口/托盘/穿透能力差异、Godot 导出、签名与商店、配置目录、触控交互和 native 抽象阻塞；iOS 是最高优先级，但候选实现版本仍待后续 `/idea` 决定。

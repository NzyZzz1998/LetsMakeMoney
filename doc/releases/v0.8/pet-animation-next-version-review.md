...
| 当前发布版 | v0.7 Beta | 已确认 |
| v0.8 状态 | 薪资与作息候选包已通过自动回归、包验证与最终真实桌面 Acceptance，可进入发布收口 | 已确认 |
...
|---|---|---|---|---|---|
| PET-REV-001 | 已收敛 | v0.8 薪资与作息已独立提交并通过最终 Acceptance | 已确认 | 宠物大改不得混入 v0.8 发布提交 | v0.8 发布收口后从干净基线进入下一版本 |
...
```mermaid
flowchart LR
    M0["M0 完成 v0.8 发布收口"] --> M1["M1 冻结宠物运行时行为基线"]
...
- v0.8 候选包、自动版本级回归与最终 Acceptance 已完成；发布收口后，再从干净基线创建下一版本分支。

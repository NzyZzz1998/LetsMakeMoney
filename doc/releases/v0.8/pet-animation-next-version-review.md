...
| 当前发布版 | v0.7 Beta | 已确认 |
| v0.8 状态 | 薪资与作息已实现，自动验证与真实 UI 手动验收通过，待提交、候选包和版本级回归 | 已确认 |
| 宠物动作大改 | 仅进入 Review，不属于当前 v0.8 已实现范围 | 已确认 |
| 工作区 | 存在 v0.8 薪资与作息有意修改，不应被宠物工作覆盖 | 已确认 |
...
|---|---|---|---|---|---|
| PET-REV-001 | Blocker | v0.8 薪资与作息虽已完成真实 UI 验收，但当前仍是未提交的独立工作区，不适合并入宠物大改 | 已确认 | 两条高风险工作线混合后难以回归和回退 | 先提交并完成 v0.8 候选包与版本级回归，再进入下一版本 |
...
```mermaid
flowchart LR
    M0["M0 提交并完成 v0.8 版本级回归"] --> M1["M1 冻结宠物运行时行为基线"]
...
- 先提交并完成 v0.8 候选包和版本级回归，再从干净基线创建下一版本分支。

...
**通过，已取得发布授权，可进入受保护分支发布收口。** 源码级门禁、受控 dirty 候选的完整主题验收、rebase 前干净候选构建、候选包合同和真实 Windows 最小身份冒烟均已通过。正式附件仍必须从 PR 合并后的干净 `main` 重新构建，旧候选不直接上传。
...
- 实现与文档提交已 rebase 到最新 `main`，仍需通过受保护分支 PR 与必需 CI。
- 最终 Release 必须从合并后的干净提交重新构建并重新锁定哈希；本节哈希只标识历史发布授权候选。

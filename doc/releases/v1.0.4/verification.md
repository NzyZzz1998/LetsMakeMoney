...
- 发布判断：通过并已发布。真实多显示器、负坐标工作区与显示器移除回落由项目所有者批准延期，记录为“暂不验证”且不阻塞 v1.0.4。
...
- WebView2Loader 大小：`160,320` 字节
- WebView2Loader SHA256：`8427B1FC58EC707813E5C0A51EB5D69397BB333250A7B891BE4D3B123F1E0F1C`
- `v1.0.4` tag object：`2e4fec17520524ac1e53a4e1bc993448d9255981`
- GitHub Release：`https://github.com/NzyZzz1998/LetsMakeMoney/releases/tag/v1.0.4`
- Release 状态：Stable，非草稿、非预发布。
- Release 附件：便携 Zip 与 `SHA256SUMS.txt`，共 2 项。
- 回下载验证：通过；Zip SHA256 与上述最终锁定值一致。
...
2. 最终发布源提交的完整验证、生产构建、打包和包体验证通过。
3. `v1.0.4` annotated tag 与 GitHub Stable Release 已创建。
4. Release 仅包含批准的便携 Zip 与 `SHA256SUMS.txt`。
5. 两个远端附件已回下载复核，Zip SHA256 与最终锁定值一致。

v1.0.4 已通过并发布；后续只进入发布后观察，不追溯修改既有 tag、Release 附件或哈希。

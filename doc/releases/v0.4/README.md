...
当前 v0.4 仍处于 `test` 分支 Beta 测试态，尚未合并 `main`，尚未打正式 v0.4 tag。
...
- 当前发布包已有测试产物，但仍需关闭手动复测项。
- 当前不应合并 `main` 或推 v0.4 tag。
...
- `V04-MAN-061` 需要人工复测通过。
- `V04-MAN-052` 已记录为 `V04-OPT-001` 后续优化项，不再作为 v0.4 当前修复项。
- `V04-MAN-072`、`V04-MAN-073` 已完成测试并记录结果。
- 自动验证、打包、包验证已重新运行。

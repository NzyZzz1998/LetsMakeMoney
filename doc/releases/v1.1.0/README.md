...
> 状态：体量连续性与拖拽调度修正已提交并生成新的干净本地候选；自动门禁通过，真实本机拖拽仍待补证，未发布。公开版本固定为 `v1.1.0`。
...
- 体量连续性在远程观察中明显改善；远程桌面不能证明指针采样和原生窗口移动手感，完整本机拖拽不得写成通过。
- 截图中断拖拽已在同源定向复验载荷关闭；当前 Zip 的本机完整重复、托盘右键命令/退出及 Settings/Modal 输入锁定仍待补证。
...
- 本机直接输入下的 idle 到 run 抓取连续性、拖拽跟手性与快速左右反转。
- 当前 Zip 中截图或失焦中断后的再次单击、再次拖拽与陈旧位移清理。
- 托盘右键命令逐项执行、退出后的进程状态与任务栏策略。
- Settings / Modal 输入锁定、正常关闭及异常关闭后的释放。
- 125% / 150% DPI 下完整 500ms 长按拖拽链路。
- 正式 EXE 内嵌坏包的安全桌面注入；现有自动隔离夹具 3/3 通过。
- Windows 10 与多显示器环境。

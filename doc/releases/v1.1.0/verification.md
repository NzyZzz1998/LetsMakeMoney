...
当前发布前结论为“tag 通过，可推送；GitHub Release 待单独授权”。Workbench 陈旧租约恢复 Mini 与原生吸附期间丢失首次收起两项竞态已经合入 clean 主线；自动回归通过，项目所有者也确认问题基本解决。左右边缘各 10 次逐次统计尚未明确完成，项目所有者接受其作为非阻塞发布后观察风险，不据此伪写 20/20 通过。`Tag publication approved = true`，`GitHub Release approved = false`。
...
- idle/基础循环与跑动姿态已通过 `256×208` 逻辑画布和帧级重组改善体量连续性；抓取点与拖拽手感已在 2026-08-17 本机直接输入验收中通过。
...
- Mini 左右边缘各连续贴边 10 次的逐次统计尚未按次数逐项记录；已有项目所有者定向反馈，且本轮明确接受为非阻塞观察项。后续若再次出现贴边失败，应保留边缘方向、释放位置、指针状态和日志事件后单独修复。

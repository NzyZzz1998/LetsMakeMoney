...
- 状态：体量重组和拖拽调度修正已提交并形成新的干净测试候选；自动门禁和本机直接输入验收通过，等待最终发布候选重建，未发布。
- 产品范围：Classic-only 可选桌面陪伴；默认 Mini；严格互斥。
- 当前阻塞：功能与真实 GUI 验收无阻塞；本次文档更新晚于候选源提交，需从最终干净提交重新构建并锁定发布哈希。公开发布仍需项目所有者单独授权。
...
- 当前候选在 100%、125%、150% DPI 下完整可见，可见/透明命中、输入锁和完整拖拽链路均已通过。
...
1. 提交本轮验收与 README 文档，从最终干净发布提交重新构建唯一候选并更新身份与哈希。
2. 对最终重建包执行身份、包体和启动冒烟，不无理由重复已经锁定的完整 GUI 验收。
3. 当前只允许推送 `test`；推送 `main`、打 tag 和创建 Release 仍需项目所有者单独授权。

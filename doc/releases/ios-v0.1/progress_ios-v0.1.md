...
| IOS01-M6 | 跨 Target 一致性与质量 | FR-001～014 | 进行中 | 9/13 |
...
- [x] IOS01-M4-014 实现 App/Widget/控制中心/App Intent 手动启停入口；共享 ActivityKit 协调器统一读取 App Group 启动上下文，App、Widget 与控制中心共用明确成功/失败反馈。
- [x] IOS01-M4-015 验证通知拒绝不禁用手动 Live Activity；纯 Swift 权限矩阵与源码合同确认通知授权只控制通知提示，不作为手动 Activity 门禁，真机系统行为并入 M4-017。
- [x] IOS01-M4-016 完成快照、时间线、状态机、权限和错误测试；Windows M4 门禁 76/76 Swift 测试通过，GitHub Actions run `29431334029` 编译正式 App 与嵌入的 Widget Extension 成功。
...
- [x] IOS01-M6-001 建立跨 Target 固定配置/时刻快照比较器。
- [x] IOS01-M6-002 验证 App、Widget、Activity、Watch 金额一致。
- [x] IOS01-M6-003 验证 App、Widget、Activity、Watch 状态和进度一致。
- [x] IOS01-M6-004 验证配置版本、节假日版本、快照身份与最近同步时间。
- [x] IOS01-M6-005 扫描硬编码用户文案、未登记字符串和乱码。
- [x] IOS01-M6-006 扫描日志、诊断和错误中的用户路径与敏感信息。
...
- [x] IOS01-M6-011 更新隐私、已知限制、构建和人工验证文档。
- [x] IOS01-M6-012 回归原型关键交互与正式文案一致性。
- [x] IOS01-M6-013 运行 M6 全量门禁并记录证据失效条件。

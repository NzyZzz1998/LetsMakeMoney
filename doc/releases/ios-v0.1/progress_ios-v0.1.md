...
- 当前状态：M3 完成 17/17，M3R 完成 14/14；M4 已完成 13/17，Widget 已覆盖桌面与锁屏 families，Live Activity 已建立版本化合同、阶段状态机、锁屏与灵动岛布局，通知权限已接入系统事实源
- 目标版本：`ios-v0.1-beta`
...
| IOS01-M4 | Widget、Live Activity 与通知 | FR-006、007、010、012 | 进行中 | 13/17 |
...
- [x] IOS01-M4-006 实现锁屏 accessory families 与窄宽度降级；内联状态与金额可退化为状态，圆形显示进度，矩形显示金额、状态和进度。
- [x] IOS01-M4-007 实现 Widget 时间线、最后更新时间、过期和未配置状态；请求 15 分钟周期刷新，快照生成 30 分钟后保留最后有效值并显示过期提示。
...
- [x] IOS01-M4-012 实现时间锚点/费率推导；按上班、午休、复工和下班锚点推导有效工作秒数、今日金额与进度，午休冻结、下班封顶、提前结束按确认时刻结算，并禁止依赖后台秒级定时器。
- [x] IOS01-M4-013 实现通知授权、拒绝、撤销和系统设置跳转；App 每次进入前台刷新系统权限事实，未请求时申请权限，拒绝后仅提供系统设置入口，不重复弹窗。
...
- 证据状态：M0-M3 与 M3R 的 Windows 合同和 iPad 主路径已收口；G3 Apple SDK 环境门禁及 M4 正式 Widget Extension 编译门禁已通过。小/中/大与锁屏内联、圆形、矩形组件已实现共享金额、状态、进度、最后更新时间和降级态，大组件已增加今日安排；时间线合同会请求周期刷新并在 30 分钟后显示过期。Live Activity 已建立版本化静态/动态合同、纯 Swift 状态机、锁屏与灵动岛布局：工作、午休、自动完成与用户确认提前结束均有确定展示，午休不显示金额，完成和提前结束不可逆；灵动岛覆盖最小、紧凑、展开和窄宽度降级。金额/进度已按时间锚点和固定费率进行纯 Swift 推导，并在初始化、阶段切换与提前结束确认时重新计算；午休冻结、复工排除午休、下班封顶。该实现不包含后台秒级定时器，也不承诺系统后台逐秒重绘金额。通知权限状态现由 `UNUserNotificationCenter` 在加载与前台恢复时读取：未请求可申请、拒绝后只跳系统设置、已允许不重复请求；通知偏好不再被持久配置冒充为系统授权事实。ActivityKit 启停协调和真机系统展示尚未实现或验证。真实 App Group 读写、系统桌面/锁屏/灵动岛展示、WidgetKit 实际调度，以及通知弹窗、拒绝、撤销和系统设置往返仍未取得设备证据；Intent 与 M5 Watch 产品能力尚未实现。
- 失效条件：schema、配置/快照模型、App/SwiftUI 源码、本地化资源、测试或 Swift 工具链版本变化时重测。

## 下一步

1. 进入 `IOS01-M4-014`，实现 App、Widget、控制中心与 App Intent 的手动 Live Activity 启停入口。
2. 保持通知权限与手动 Live Activity 能力解耦；拒绝通知不得禁用手动启停，最终设备行为由 M4-015 与 M4-017 补证。

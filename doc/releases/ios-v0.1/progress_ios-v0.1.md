...
- 当前状态：M3 完成 17/17，M3R 完成 14/14；M4 已完成 6/17，小/中/大及三种锁屏组件已实现金额、状态、进度和紧凑降级态，大组件额外展示今日安排
- 目标版本：`ios-v0.1-beta`
...
| IOS01-M4 | Widget、Live Activity 与通知 | FR-006、007、010、012 | 进行中 | 6/17 |
...
- 证据状态：M0-M3 与 M3R 的 Windows 合同和 iPad 主路径已收口；G3 Apple SDK 环境门禁及 M4 正式 Widget Extension 编译门禁已通过。小/中/大与锁屏内联、圆形、矩形组件已实现共享金额、状态、进度及降级态，大组件已增加今日安排；真实 App Group 读写、系统桌面/锁屏展示与刷新仍未取得设备证据。Live Activity、通知、Intent 与 M5 Watch 产品能力尚未实现。
- 失效条件：schema、配置/快照模型、App/SwiftUI 源码、本地化资源、测试或 Swift 工具链版本变化时重测。

## 下一步

1. 进入 `IOS01-M4-007`，实现 Widget 时间线、最后更新时间和过期策略，并继续复用现有快照与降级状态。
2. M4-007 关闭后进入 Activity Attributes 与状态机，不提前实现持续后台秒级定时器。

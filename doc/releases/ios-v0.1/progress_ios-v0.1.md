...
- 当前状态：M0 部分通过，等待 iPad Swift Playgrounds 人工补证
- 目标版本：`ios-v0.1-beta`
- 目标分支：`ios-main`（已在独立 worktree 创建，尚未推送）
...
| 里程碑 | 模块 | 主要 FR | 状态 | 完成度 |
| --- | --- | --- | --- | --- |
| IOS01-M0 | 基线、分支与可行性 | FR-012～FR-014 | 部分通过 | 9/10 |
...
| iPad Swift Playgrounds 尚未真机验证 | M0-007、G2 | 需项目所有者按人工清单补证；未补证前 M0 不写完成 |
| 2027 官方节假日数据可用性待核实 | 完整离线数据集 | 未有官方数据时标记未覆盖，禁止猜测 |

## 最近验证

- 验证时间：2026-07-13
- 验证对象：Windows v0.7 基线、Apple M0 目录与文档合同、M0 检查器。
- 验证方式：`verify_v07.ps1`、`check_docs_status.ps1`、`scripts/apple/check_ios_m0.ps1` 及其反例测试。
- 结果：Windows 基线通过；M0 必需文件 19/19；扫描文件 22；真实项目与干净夹具通过；绝对用户路径和缺失文件反例按预期失败。
- 证据状态：新测。
- 失效条件：Apple 目录契约、标识模板、M0 文档或检查器变化时重测。

## 下一步

1. 在 iPad Pro M4 上按 `playgrounds-verification.md` 完成 `IOS01-M0-007`。
2. 将 Swift Playgrounds 版本、各步骤结果和截图文件名回填到人工验证文档。
3. G2 通过或明确为部分通过后关闭 M0，再由项目所有者授权进入 SalaryCore M1。

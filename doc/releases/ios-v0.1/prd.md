...
- 当前状态：2026-07-14 首次引导增量 PRD 与正式原型已获项目所有者确认，进入增量开发承接
- 目标版本：`ios-v0.1-beta`
- 产品线：Apple 平台独立产品线
- 目标分支：`ios-main`（开发承接阶段创建）
- 上游来源：2026-07-13 iOS 产品探索与需求确认；2026-07-14 iPad 实机首次引导反馈
...
- 开发授权：已获得；原开发承接继续有效，本次首次引导变更以 `IOS01-M3R` 增量批次更新开发计划与进度
...
- 首次引导按 08:00 上班、12:00 午休开始、2 小时午休和 8 小时有效工时推算 18:00 下班，并允许同页微调。
...
- 用户不直接填写日期，而是回答“本周是大周还是小周”。
- 大周定义为本周周六上班，小周定义为本周周六休息。
- 底层 `alternatingAnchor` 仍表示一个休息周的周六：选择小周时写入本周周六，选择大周时写入下周周六。
- 本周以设备当前日历、时区和自然周计算；周六当天选择时，本周周六即当天。
- 重新配置时依据锚点与本周周六的周差反推本周类型；无法可靠反推时要求用户重新选择，不猜测。
- 由锚点按自然周奇偶推导大周和小周；缺少合法选择时禁止保存大小周配置。
...
- 类型：配置 / 数据
- 入口：首次引导、设置、日历日期详情
- 默认：双休；引导默认输入 08:00 上班、2 小时午休、12:00 午休开始，并推算 14:00 午休结束、18:00 下班和 8 小时有效工时
- 时间输入：使用系统时间选择组件；不接受自由文本、英文字符或手写冒号
- 智能推算：`午休结束 = 午休开始 + 午休时长`，`下班时间 = 上班时间 + 8 小时有效工作 + 午休时长`
- 详细调整：修改午休开始时保持午休时长并更新结束；修改午休结束时保持午休时长并反推开始；修改上下班后按最终四个时间重算有效工时，不再强制保持 8 小时
- 午休时长：30 分钟步进，范围 0-3 小时；零午休时开始与结束相同，界面显示“无午休”，计算不扣减午休
- 保存：只有 `上班 < 午休开始 <= 午休结束 <= 下班`、不跨日且工作日规则合法时写入
- 取消/关闭：丢弃草稿，不改变有效配置
- 异常：午休越界、推算跨日、大小周未选择或锚点无法反推时阻止保存并定位错误；自动联动失败时保留最后一组有效安排
- 影响范围：配置模型既有字段、SalaryCore 大小周解析和作息草稿纯计算、首次引导、日历 UI、计算内核、Widget/Activity/Watch 快照；不新增配置字段，不升级 schema，无数据库影响
- 验收：所有配置组合可保存、取消、重启恢复，并产生可解释计算结果

### FR-003 三步首次引导

- 类型：UI / 交互
- 稳定结构：固定中文标题与无文字进度条、可滚动内容区、底部操作区；横屏唤起键盘时标题和进度不随内容上浮，不显示 `onboarding.step.*` 原始键
- 步骤一：月薪、币种、休息模式；选择大小周时询问“本周是大周还是小周”，不显示日期输入
- 月薪输入：初值显示 `0.00`；零值首次聚焦清空，已有非零配置聚焦时保留；允许整数或最多两位小数，编辑时不插入千位分隔符，失焦或进入下一步时规范化；非法文本保留并阻止前进
- 步骤二：先选择上班时间和午休时长，同页即时展示推算后的上班、午休、下班与有效工时；可展开或直接调整四个专用时间控件，按 FR-002 规则联动
- 步骤三：本月工作日、日薪和今日示例预览，确认后保存
- 下一步：校验当前步骤；失败时停留并显示字段错误
- 上一步：保留草稿
- 取消/关闭：首次启动时要求确认退出，退出后下次仍进入引导；重新配置时取消不改变配置
- 完成失败：保留草稿并允许重试
- 影响范围：`OnboardingView` 布局与焦点管理、金额编辑状态、系统时间选择控件、SalaryCore 的大小周解析和作息草稿、配置事务、计算预览、本地化文案与 iPad 横竖屏；不修改 Settings，不影响 Widget 展示直到完成保存
- 验收：完整、返回、取消、关闭、失败重试和重启路径均有测试；iPad 横屏键盘、金额光标习惯、大小周自然语言、时间联动均有真实设备证据
...
首次引导只改变上述字段的输入和推算方式，不新增字段、不升级 `schemaVersion`：

- `monthlySalaryMinor` 由独立金额编辑状态转换，避免每次按键都格式化并移动光标。
- `alternatingAnchor` 继续保存休息周周六的 `yyyy-MM-dd`，界面只暴露大周/小周语义。
- `workStart`、`lunchStart`、`lunchEnd`、`workEnd` 继续保存 `HH:mm`；时间选择器在持久化边界转换。
- `standardWorkSeconds` 使用最终四个时间重新计算；零午休时 `lunchStart == lunchEnd` 合法。
...
- 首次引导月薪默认显示 `0.00`，零值首次聚焦清空；编辑过程中不强制补小数或插入千位分隔符。
- 首次引导只显示中文主标题和无文字进度，不显示步骤英文或本地化资源键。
- 大小周文案必须解释“大周：本周周六上班；小周：本周周六休息”。
- 作息时间必须使用专用时间控件；午休时长使用 30 分钟步进控件，并即时展示推算结果。
...
- 首次引导原型必须可操作金额输入、休息模式、大周/小周、上班时间、午休时长和详细时间联动，并展示错误、返回、取消与完成状态。
- 支持浅色/深色切换和设备视图切换。
...
- iPad 横竖屏、分屏和窗口尺寸变化。
- iPad 横屏唤起键盘后，中文标题、无文字进度条、金额输入和底部操作区无重叠；竖屏行为不回退。
- 月薪输入分别覆盖整数、一位小数、两位小数和非法第三位小数，光标不被持续格式化干扰。
- 大小周在大周/小周两种选择下生成正确休息周周六锚点。
- 第二页专用时间控件、默认推算、午休开始/结束双向联动和上下班重算有效工时符合规则。
- 跨日、锁屏、重启、低电量和时区变化。
...
- 无配置丢失、无法恢复或隐私泄露问题。
- 所有人工必测项有实际设备证据。
- 首次引导五项实机反馈全部关闭，且旧配置可重新进入引导并正确解释。
...
- [x] 项目所有者完成 2026-07-14 增量 PRD 与正式原型复核。

## 18. 下一阶段建议

增量 PRD 与正式原型确认后，更新既有开发承接文档：

...
| 素材候选 | Classic Pro（默认候选）、多多 Pro（正式可选） |
...
- Classic Pro 经影子接入和门禁后成为默认候选；多多成为正式可选宠物。
...
**目标**：Classic Pro 成为默认橘猫第一候选，但不未经验证直接替换。

**阶段**：

1. 影子导入：包可解析、缓存和预览，但用户默认仍为 v0.8 橘猫。
2. 对照验证：同一业务状态和交互脚本对比尺寸、锚点、帧时长、清晰度、CPU 和命中区。
...
**动作覆盖**：Classic 必须提供 working/awake_rest/sleeping 三基础状态下可辨认的 single 语义，以及通用 `run_prepare/running_left/running_right/run_settle`。sleeping 单击只做耳朵或尾巴轻动，不强制完全唤醒；业务状态不因互动永久改变。多多首轮允许使用显式通用回退，Classic 观感门禁通过后再决定是否补生成专属动作。
...
**专属补生成条件**：Classic 默认切换门禁通过；多多共用映射真实桌面验收后，存在用户可见的身份错位、动作辨识不足或轮廓异常；有明确动作规格、验收帧数和回滚方案。否则不为追求数量批量生成。

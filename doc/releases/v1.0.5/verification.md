...
| 项目 | 状态 |
| --- | --- |
| 当前阶段 | 独立 ACC 已执行并复开 V105-BUG-001；最小代码修复及自动回归已通过，等待新 clean 候选复验 |
| M0 结论 | 通过，8/8 |
| M1 结论 | 通过，8/8 |
...
| v1.0.5 候选 | 唯一 clean M6 候选自动聚合通过，但独立 ACC 未通过；不可发布 |
| 构建 / 打包 | TypeScript/Vite/Rust、隔离打包和 candidate 包验证通过；已生成独立验收候选，未生成正式发布包 |
| 发布判断 | 不可发布；自动修复尚未取得新候选真实 Windows 证据 |

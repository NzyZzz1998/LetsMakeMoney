...
| M6 结论 | 部分完成，9/10；受控 dirty 候选完整聚合通过，干净提交候选未生成 |
| 用户可见实现 | M3 至 M5 范围已实现：Mini 隐私行为、日历复合状态与三窗单一表面 |
| v1.0.5 候选 | 受控 dirty M6 候选已锁定并通过聚合；禁止发布 |
| 构建 / 打包 | TypeScript/Vite/Rust、隔离打包和 candidate 包验证通过；未生成正式发布包 |
| 发布判断 | 不可发布；干净提交候选与独立 ACC 尚未完成 |
...
- 源码 HEAD：`8a63da7836fb24c3b7f8ff12f896ac40571adeb7`。
- 源码状态：`dirty`。
- Zip：`LetsMakeMoney-v1.0.5-windows-x86_64.zip`，3,231,538 字节。
...
- 候选明确记录 `source_tree_dirty=true` 和独立验收未完成；**不可发布**。
- M6 完整聚合门禁已对上述锁定对象通过；它仍不能代替干净提交候选或独立验收。

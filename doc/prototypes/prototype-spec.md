...
- 当前状态：v1.0 Stable 已发布；v1.0.1 完整 PRD 与原型增量待确认
- 当前稳定基线：Windows v1.0 Stable
- 当前原型：`doc/prototypes/v1.0/index.html`，包含 v1.0.1 可信度修复增量
- 当前分支：`main`
- 历史原型索引：`doc/prototypes/index.html`
- 需求入口：`doc/releases/v1.0.1/prd.md`
- 最后更新：2026-07-27

> 第 2-16 节记录 v0.5-v0.9 的历史设计与验收合同，不再代表当前产品。第 17 节记录 v1.0 基线，第 18 节及 `doc/prototypes/v1.0/` 记录 v1.0.1 增量。
...
## 17. Windows v1.0 无宠物稳定版原型

`doc/prototypes/v1.0/index.html` 是 v1.0 的高保真交互事实源。v1.0 不继承 v0.9 的宠物窗口、Panel 或旧 Settings 结构，而是围绕以下产品链路重建：

1. 启动后显示 344×120 的迷你收入视图；
2. 打开 820×620 的今日/日历工作台；
3. 首次使用进入 760×560 的三步配置 Wizard；
4. 通过 720×540 的任务化 Settings 维护收入、日历、窗口和本地数据；
5. 关闭窗口后驻留托盘，并允许恢复上次窗口；
6. 保存失败、配置恢复、更新失败等状态不得污染旧配置。

顶部验证控制条只用于原型审核，不进入真实产品。正式实现应复用原型中的颜色、字体、间距、窗口比例和组件状态，但不得把网页截图作为运行时界面。

技术路线已收敛为 Rust + Tauri + TypeScript/React。WinUI 3 不进入 v1.0，Godot 仅作为 v0.9 发布回退基线。真实 Windows 通知区左键、单实例窗口复用以及 125%/150% DPI 仍需由可执行候选完成验收。
...
## 18. Windows v1.0.1 日历与收入可信度增量

v1.0.1 不重做 v1.0 的窗口结构和视觉系统，仅在现有 `doc/prototypes/v1.0/` 中增加以下可交互状态：

1. 日历数据范围明确为中国大陆 2025—2026，数据随应用离线提供；
2. 日历包含加载中、可用、使用旧数据、首次加载失败、重试和不支持年份状态；
3. 跨月切换时标题与日期网格作为一个结果整体更新，不允许新标题配旧网格；
4. 用户先选中日期，再通过“调整所选日期”进入单日事务；
5. 单日事务支持跟随自动判断、设为工作日、设为带薪休息和设为不带薪休息；
6. 带薪/不带薪休息只对底层自动结果为工作日的日期开放；自动休息日显示解释并禁用两个请假选项；
7. “应用”成功后以工作日、带薪休息或不带薪休息标记展示，并触发全链路重算反馈；
8. “取消”、右上角关闭和 `Esc` 不产生配置变化；
9. 模拟写入失败时弹层保留选择，显示可读错误，旧日历不变；
10. Settings 日历页只展示数据范围和优先级，不保留无行为绑定的“允许手动调整”开关；
11. 正常可用状态直接展示日历，不显示“官方日历已就绪”状态块；状态块只用于加载、旧数据、失败和不支持年份。

原型顶部的“日历状态”选择器是设计审核工具，不进入正式应用。真实年度日期、来源、哈希、金额公式、跨夜归属和权威同步频率以 `doc/releases/v1.0.1/prd.md` 为准。

...
**当前版本**: v0.4 Beta  
**当前分支**: `test`  
**当前阶段**: Beta 测试态，尚未合并 `main`，尚未打 v0.4 tag
...
- `V04-MAN-061`: 托盘左键隐藏 / 显示后的纯桌宠恢复策略已修复，仍需人工复测。
- `V04-MAN-072`: 连续单击、双击、右键各 5 次仍未测试。
- `V04-MAN-073`: `debug.log` 稳定性检查仍未测试。
- `V04-OPT-001` / `V04-MAN-052`: Wizard 薪资页控件视觉与 Settings 一致性多轮修复后体感无明显变化，已暂缓并转入后续优化清单，不再作为当前继续修补项。

修复完成后，失败项只能先标为“已修复待复测”，不能直接标为“通过”；明确转后续优化的项目不再反复作为当前 bug 修补。

## 当前发布产物状态

v0.4 Beta 发布包已经形成测试产物：
...
当前发布包仍属于测试态产物。任何代码、资源或发布说明修复后，都需要重新运行 `scripts/package_v04.ps1` 和 `scripts/verify_v04_package.ps1`。
...
- [doc/releases/v0.4/verification.md](releases/v0.4/verification.md) 仍包含待复测和未测试项；不要把 v0.4 当作完成态。
- `releases/v0.4-beta-notes.md` 对最新 UI / Wizard 修复可能滞后；正式发布前需要同步。

## 下一步

1. 按 [doc/releases/v0.4/verification.md](releases/v0.4/verification.md) 复测待验证项。
2. 根据复测结果更新 [doc/releases/v0.4/status.md](releases/v0.4/status.md)、[doc/releases/v0.4/progress.md](releases/v0.4/progress.md) 和 [doc/releases/v0.4/verification.md](releases/v0.4/verification.md)。

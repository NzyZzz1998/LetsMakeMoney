...
| 当前公开 tag | `v1.0.4` |
| 当前开发版本 | Windows v1.0.5；V105-BUG-001 最小代码修复及自动回归已通过，等待新 clean 候选 ACC |
| 下一版本候选 | Windows v1.0.5；必须完成最小修复并重新构建 clean 候选 |
| v1.0.1 阶段 | 已发布 |
| v1.0.2 阶段 | 已发布 / 发布后观察 |
| v1.0.3 阶段 | 已发布 / 发布后观察 |
...
| 发布阻塞 | v1.0.4 无；v1.0.5 修复尚未取得新 clean 候选真实桌面证据，当前不可发布 |
| 最后更新 | 2026-08-01 |
...
独立 ACC 已在 Windows 11、100% DPI、单显示器环境执行，结论为**未通过**：Mini 贴到右侧边缘后，保持焦点时不会首次自动收起，只有窗口失焦后才收起。日志确认拖拽结束后的 `focus_inside` 锁阻止了收起调度，V105-BUG-001 已复开并成为发布阻塞。Workbench 关闭、Wizard、Settings、日期调整、日历复合状态和用户环境恢复通过或部分通过；通知区真实鼠标、独立 125%/150% DPI 与多显示器仍需由修复后的新候选补证。本候选**不可发布**：
...
v1.0.5 独立 ACC 已复开 V105-BUG-001；最小代码修复和自动回归现已通过。下一步从包含修复与红灯测试的新干净提交重新构建候选，并重跑受影响的 Mini、隐私竖条、通知区找回、v1.0.4 回归和发布判断。旧候选及其哈希不得用于发布；新候选真实复验完成前仍不可发布。

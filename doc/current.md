...
| 当前公开 tag | `v1.0.2` |
| 当前开发版本 | Windows v1.0.3 独立 Acceptance 进行中 |
| v1.0.1 阶段 | 已发布 |
| v1.0.2 阶段 | 已发布 / 发布后观察 |
| v1.0.3 阶段 | 实现与自动门禁完成；独立 Acceptance 进行中，当前不可发布 |
| 技术栈 | Rust + Tauri + TypeScript/React |
| 产品形态 | 无宠物、本地优先的 Windows 收入进度工具 |
| 发布阻塞 | 当前候选来自脏工作树；必须从干净发布提交重新构建并锁定最终哈希 |
| 最后更新 | 2026-07-30 |
...
v1.0.3 的发布后 Review、Idea、性能技术 Spike、PRD、开发承接和业务实现已经完成。聚合自动验证、年度数据验证、权威同步、窗口生命周期、Rust 回归、打包及包体验证通过；当前处于独立 Acceptance。
...
当前完成度为 `61/62`。官方/估算日历、日期调整、Settings、首次启动 Wizard、隐藏恢复、连续 10 次显隐、真实时区切换、真实系统时间前后跳变、通知区真实鼠标左键、两条真实 S3 睡眠跨边界路径和修正版候选 120 分钟稳定运行已经从新解压候选取得证据。原候选的稳定性采样发现隐藏 Workbench 后持续高 CPU，已定位为原生 WebView2 未挂起；修正版连续运行 `7201.27` 秒并通过 CPU、内存、日志与同步频率门禁，`V103-BUG-001` 已关闭。系统时间前拨、后拨和恢复经项目所有者确认 UAC 后完成，阶段在数秒内收敛，时区、配置、进程和系统时间均已恢复。工作跨休息、休息跨恢复工作均已真实进入 S3 并正确收敛。当前候选由脏工作树构建，只能用于验收，不能直接发布。
...
系统与功能门禁已经闭合。下一步先提交有意变更，再从干净提交重新构建发布候选、更新最终哈希并执行发布身份核对。

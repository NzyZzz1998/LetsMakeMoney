...
**当前阶段**：v0.8 Beta 已通过 PR #3 合并至 `main`，annotated tag 与 GitHub Pre-release 已发布；v0.9 已完成多多 S5.5 接入和定向修复，独立验收部分通过后冻结归档，不作为公开稳定版本
...
2026-07-22，v0.9 已完成工资与作息口径、Settings/Wizard 高保真生产级还原、Panel/今日详情、通用宠物包运行时、动画状态机、输入仲裁、业务事件层和配套自动门禁。多多 S5.5 已接入，Classic 保持兼容回退；`V09-BUG-002/003/004/005` 已关闭。锁定候选 Zip SHA256 为 `65A04A1BAFF6681FF335DD2966A528E6BD6517A81232BC107EFAF5AF42C9F685`。独立验收结论为“部分通过”，项目所有者接受当前视觉体验债并冻结本版；真实 125%/150% DPI、通知区鼠标、500ms 长按跑动、两套宠物完整观感、受控损坏包桌面回退和两小时稳定运行保留为暂不验证。该状态不等于完整验收通过，也不授权公开稳定发布。
...
- `doc/releases/v0.9/progress_v0.9.md`：冻结中的下一版本开发状态、验收结论和证据入口。
...
v0.9 不再继续开发或补签验收，冻结候选仅用于归档和后续版本回归基线。若未来决定重新发布 v0.9，必须重新打开范围、生成新候选并从头执行独立 `/acceptance`；否则下一版本应重新收敛视觉质量和宠物动画目标。任何情况下都不得把 v0.9 的“部分通过”写成完整验收通过，也不得覆盖 v0.8 的当前公开发布事实。

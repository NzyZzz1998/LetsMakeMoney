...
- 独立 `LetsMakeMoney-ios` 仓库存在并维护 `ios-main`，但 Windows 仓库中的 2 份同名原型和 5 份核心过程文档均与独立仓库内容不同；名称相同不足以证明完整接管。
...
## 2026-07-31 独立干净候选最终验收

### 候选重建

- 在隔离新鲜 clone 中形成干净提交 `661b0f748798e28f7999eac23532aa7ed7510640`。
- 聚合验证、TypeScript/Vite、Rust test/format/clippy、正式打包和包体验证全部通过。
...
- 100%/125%/150% DPI：Mini、Workbench 和 Settings 无裁切、重叠或模糊。
- 左右工作区边缘自动收起、3px 隐私条、唤回、移开收回、关闭设置和重启持久化：通过。
- 深色主题跨窗口即时同步和重启持久化：通过。
- Windows 动画效果关闭后，Mini 收起无崩溃、闪烁或残留过渡；系统设置已恢复。
- 全新隔离配置完成 Wizard 三步、返回、取消、关闭和完成。
- Workbench 今日信息、2026 官方日历跨月和日期调整取消事务：通过。

### 剩余证据边界

- 当前环境只有一台显示器；真实多显示器、负坐标与显示器移除回落待人工补证。
- Computer Use 无法可靠命中 Windows 通知区图标；真实通知区左键隐藏、恢复和窗口找回待人工补证。
- Mini WebView 内右键显示 WebView2 浏览器上下文菜单，记录为非阻塞体验观察。
- 原始截图未建立持久外部归档；仓库只保留脱敏候选身份、检查结论和语义事件计数。

### 环境恢复与判断

- 普通用户和 Codex 隔离目录的配置、前一版本配置与日志哈希均恢复一致。
- Windows 系统缩放恢复为 100%，动画效果恢复开启。
- 全部 LetsMakeMoney 进程已停止。
- 当前无新增产品缺陷；整体验收仍为部分通过。通知区和多显示器门禁完成或明确延期后，才能进入发布收口。

## 2026-07-31 通知区真实鼠标补证

### 验证对象

- 使用隔离干净候选 EXE，SHA256 为 `EB0FA82F5506B775F03774E429A12DB66F838E705B608F4A7664D6F9243830DF`。
- 实际运行路径位于独立新解压目录，不使用开发构建目录。
- 项目所有者负责 Windows 通知区真实鼠标输入，Computer Use 负责窗口前后状态观察。

### 真实结果

1. 第一次左键点击通知区图标后，Mini 从窗口列表消失，原进程持续运行。
2. 第二次左键点击后，同一 Mini 窗口恢复，内容正常，进程未重启。
3. 脱敏日志顺序为：
   `tray.left_click action=toggle_mini` →
   `window.hidden label=mini` →
   `tray.left_click action=toggle_mini` →
   `window.shown label=mini` →
   `earnings.authoritative_sync.requested reason=window_shown`。
4. 本轮没有执行配置保存；补证结束后全部 LetsMakeMoney 进程已停止，进程数为 `0`。
5. 通知区补证向普通用户 `debug.log` 追加了上述语义事件，因此不沿用主验收结束时“日志哈希未变化”的描述。

### 收口判断

- Windows 通知区真实鼠标隐藏、恢复、进程连续性和恢复后权威同步：通过。
- M2 完成 8/8，ACC 完成 11/12。
- 当前只剩真实多显示器、负坐标工作区和显示器移除回落硬件证据；最终发布提交仍需重新构建并锁定哈希。

## 2026-07-31 多显示器延期决策与独立验收收口

- 当前验收设备只有一台显示器，无法取得真实多显示器、负坐标工作区和显示器移除后的安全回落证据。
- 项目所有者明确批准将上述硬件场景延期验证。本项结论为“暂不验证”，不得追溯改写为“通过”。
- 延期项不再阻塞 v1.0.4；M6 按任务处置完成 14/14，ACC 完成 12/12，总体完成 74/74。
- 隔离干净提交候选 `661b0f748798e28f7999eac23532aa7ed7510640` 的自动门禁、100%/125%/150% DPI、Mini 隐私贴边、深色主题、减少动态效果、首次配置、日历事务及通知区真实鼠标找回均已通过。
- 独立验收结论更新为“通过 / 可进入发布收口”，但当前候选仍不是最终发布产物。
- 发布收口仍需确定最终发布源提交，从干净提交重新构建、重新锁定 Zip/EXE/WebView2Loader 哈希，并按变更影响复核运行门禁。
- 本次状态收口不构成 commit、push、tag、GitHub Release 或其他远端写操作授权。

...
### 2026-07-11 / V07-C1-C4 Windows 安装与用户确认更新

- 固定 Inno Setup 6.7.3，建立当前用户安装、许可页面、快捷方式、运行检测、默认保留数据和显式删除数据门禁。
- 签名脚本只读取安全环境变量；没有真实证书时安装器可用于受控测试，但 manifest 明确 `public_release_eligible=false`。
- 更新服务采用 GitHub Release、稳定/测试通道、6 小时节流、SHA256 与原生 WinVerifyTrust 双校验；不保存 GitHub 凭据，不接受非 GitHub 来源或便携 Zip 自覆盖。
- 安装前创建配置 `.pre-update` 备份并关闭托盘；启动安装器失败时恢复托盘和穿透。真实签名、SmartScreen、安装取消/修复/升级留给 Acceptance。

### 2026-07-11 / V07-D/E 规划、公开治理与候选包

- 完成 iOS 优先的平台规划、主题 token/回退规划和宠物 manifest/素材许可规划，均不增加 v0.7 业务能力。
- 完成中英 README、贡献指南、行为准则、安全政策、Issue/PR 模板、仓库设置清单；第三方 Actions 固定不可变 commit。
- v0.7 版本源切换为 `0.7-beta`，生成含许可证的便携 Zip并通过 smoke；生成未签名测试安装器并明确禁止作为公开附件。

...
- 自动化验证：V105-M0 至 M4 聚合门禁通过；M4 日历矩阵、前端构建和 Rust 门禁通过。
- 真实桌面验证：M3 左右贴边、指针/点击找回、移开收回、普通 focus 和 Workbench 关闭回归通过。
- 日历真实桌面验证：official、estimated、今天/选中/业务复合状态和浅/深主题通过。
- 打包验证：仅生成受控 dirty M3 验证候选，未生成正式 v1.0.5 发布候选。
- 未覆盖项：M5 真实壳 Spike、M6 聚合候选、通知区真实鼠标、键盘真实焦点、日历 125%/150% 真实 DPI、多显示器和独立 ACC。
...
- 下一阶段建议：执行 V105-M5 三窗单一表面真实壳 Spike；若门禁失败，完整保留 v1.0.4 表面。

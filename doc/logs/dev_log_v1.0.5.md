...
### 2026-08-01 独立 ACC 后 V105-BUG-001 最小修复

- 独立 ACC 根因复核：原生 dock 结果正确，pointer intent 也已清理；真正阻塞首次收起的是拖拽释放后仍由 WebView 持有的 `focus_inside` 锁。
- TDD 红灯：在现有 Mini 行为套件新增左右边缘“保持焦点也需首次收起”用例，并增加 menu/modal 锁保护；修复前稳定失败于左侧贴边未创建 timer。
- 最小实现：只在原生确认贴边成功后，将拖拽遗留焦点所有权交还给隐私状态机；不清理 menu/modal，不改变浮动窗口、失败回退、托盘找回或收入/日历/配置逻辑。
- 定向验证：新增回归由红转绿，左右边缘和交互锁保护通过。
- 扩大验证：架构行为与结构门禁全部通过；TypeScript strict、Vite production build、Rust `54/54`、fmt 和 clippy 全部通过。
- 停止点：尚未重建 clean 候选，也未执行真实 Windows 定向 ACC；旧候选及旧 GUI 证据继续保持失败事实。

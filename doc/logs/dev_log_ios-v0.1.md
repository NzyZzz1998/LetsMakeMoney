...
### 2026-07-16 M6 跨 Target 事实一致性

- 以现有 `SharedSnapshotBundle` 为唯一输入，不在 App、Widget、Live Activity 或 Watch 中重复执行工资计算。
- 新增 `CrossTargetSnapshotComparator`，把四个 Target 投影成同构事实并输出结构化报告；报告分别保留缺失 Target、重复 Target、快照身份、生成时间、金额、状态、进度、配置版本和节假日版本问题。
- `lastSynchronizedAt` 只在四端生成时间一致时成立；存在漂移时返回空并保留具体问题，避免用“最新时间”掩盖旧端数据。
- 定向 Swift 测试 2/2 通过，覆盖完全一致和多维漂移反例；对应 `IOS01-M6-001` 至 `IOS01-M6-004` 已关闭。

### 2026-07-16 M6 文案与隐私静态门禁

- 新增 Apple 产品质量扫描器，检查未登记本地化 key、硬编码英文用户文案、乱码标记、用户绝对路径、邮箱、疑似秘密和日志中的原始错误描述；输出只包含规则、相对路径和行号，不回显敏感内容。
- 反例测试 4/4 通过，真实产品树的质量扫描与既有本地化扫描均通过。
- 扫描发现 Live Activity 启动失败曾把原始系统错误描述写入日志；现改为稳定 `reasonCode` / `errorCode`，仍保留失败反馈和结构化诊断，不再泄露底层上下文。
- 对应 `IOS01-M6-005`、`IOS01-M6-006` 已关闭。

### 2026-07-16 M6 文档、原型合同与自动门禁收口

- 新增原型/正式文案合同校验，锁定今日、日历、设置、引导步骤和关键按钮；校验首次执行发现两处引导标题漂移，原型修正后测试 3/3 与真实扫描均通过。
...
- 自动化验证：M0-M6 自动门禁通过；SalaryCore 86/86 测试通过，GitHub macOS 已提供正式 App、Widget Extension 与 Watch 相关 Apple SDK 编译证据。
- 手动验证：iPad 主 App 路径与 M3R 已完成；M4 iPhone 系统行为、M5 Series 10 行为以及 M6 深浅色/辅助功能/系统状态矩阵仍待真机补证。
- 打包验证：Apple Beta 候选归入 M7，尚未开始签名归档和候选包验收。
- 未覆盖项：正式签名、App Group 真机共享、Live Activity 系统限流、Watch 真机复杂功能以及 M6 四项真机体验矩阵。

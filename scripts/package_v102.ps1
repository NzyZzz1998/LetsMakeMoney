...
$releaseRoot = Join-Path $root "releases\v1.0.2"
$packageName = "LetsMakeMoney-v1.0.2-windows-x86_64"
...
try {
    ...
    $buildInfo = [ordered]@{
        ...
        calendar_manifest_sha256 = (Get-FileHash (Join-Path $calendarTarget "manifest.json") -Algorithm SHA256).Hash
        calendar_2025_sha256 = (Get-FileHash (Join-Path $calendarTarget "cn-2025.json") -Algorithm SHA256).Hash
        calendar_2026_sha256 = (Get-FileHash (Join-Path $calendarTarget "cn-2026.json") -Algorithm SHA256).Hash
    }

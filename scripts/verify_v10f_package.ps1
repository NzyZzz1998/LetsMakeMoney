...
if ($ExpectedVersion -ne "1.0.8") {
    throw "v1.0.8 package verification cannot validate version '$ExpectedVersion'."
}
...
$arguments = @(
    (Join-Path $AppRoot "tests\verify_v10f_package.py"),
    "--mode", $Mode,
...
if ($LASTEXITCODE -ne 0) {
    throw "v1.0.8 $Mode package identity verification failed with exit code $LASTEXITCODE"
}

...
if (-not $PackagePath) {
    $PackagePath = Join-Path $releaseRoot "LetsMakeMoney-v1.0.2-windows-x86_64.zip"
}
...
$extractRoot = Join-Path $root ".tmp_package_v102"
...
try {
    ...
    if ($buildInfo.version -ne "1.0.2") { throw "BUILD-INFO version is not 1.0.2." }
    ...
    Write-Host "V1.0.2 package verification passed." -ForegroundColor Green

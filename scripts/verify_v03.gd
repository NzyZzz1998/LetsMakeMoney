...
func _check_config_defaults_and_migration() -> void:
	...
	_assert(merged.get("pet_id") == "cat_orange_v1", "v0.3 should default to orange cat assets")
...
func _check_pure_pet_mode_gate() -> void:
	...
	_assert(main_script.contains("Platform.set_taskbar_visible(get_window(), true)"), "Main should restore taskbar visibility on fallback")
...
func _check_settings_v03_controls() -> void:
	...
	_assert(settings_script.contains("TopActionRow"), "Settings should keep a top action row visible across tabs")
	...
	_assert(settings_script.contains("action_row.visible = false"), "Settings should hide the deprecated bottom custom action row")
	...
	_assert(drag_script.contains("popup.add_item(\"隐藏到托盘\", 600)"), "Context menu should expose a manual hide-to-tray entry")
	...
	_assert(drag_script.contains("_window.borderless = false"), "DragResizeSystem should make the host window readable while modal settings are open")
	...
	_assert(main_script.contains("_apply_viewport_transparency(false)"), "Main should disable transparent viewport while modal settings are open")
	_assert(main_script.contains("_set_primary_content_visible(false)"), "Main should hide pet and panel behind modal settings")
	_assert(main_script.contains("if _modal_open:"), "Main should keep passthrough disabled while modal settings are open")
	_assert(main_script.contains("PET_HIT_OFFSET"), "Main should use a tighter pet hit rect offset instead of the full old window area")
	_assert(main_script.contains("PET_HIT_OFFSET := Vector2(62, 80)"), "Main native passthrough pet hit rect should avoid orange cat transparent effects")
	_assert(main_script.contains("PET_HIT_SIZE := Vector2(98, 86)"), "Main native passthrough pet hit rect should stay close to visible orange cat core")
	_assert(main_script.contains("PET_HIT_MAX_SCALE := 1.25"), "Main native passthrough pet hit rect should cap high user scale")
	_assert(main_script.contains("PET_CONTEXT_SIZE := Vector2(172, 186)"), "Main should keep a larger pet context rect for right-click menu reliability")
	...
	_assert(main_script.contains("pet_hit_center"), "Main should keep capped pet hit rect centered on the scaled pet")
	_assert(main_script.contains("pet_hit_scale: float = min(s, PET_HIT_MAX_SCALE)"), "Main should avoid scaling pet passthrough rect without an upper bound")
	...
	_assert(pet_script.contains("HIT_RECT := Rect2(Vector2(62, 80), Vector2(98, 86))"), "Pet input hit rect should match the tighter visible orange cat core")
	...
	_assert(pet_script.contains("PetManager.current_interaction != PetManager.PetInteraction.CLICKED_HOLD"), "Pet should keep clicked_hold feedback while dragging after a long press")
	_assert(pet_scene.contains("size = Vector2(98, 86)"), "Pet collision shape should not use an oversized click box")

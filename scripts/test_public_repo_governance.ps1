...
foreach($token in @('v0.7 Beta','Windows x86_64','GitHub Releases','CONTRIBUTING.md','SECURITY.md','ASSETS_LICENSE.md','run_ci_verification.ps1')){Need $zh.Contains($token) "Chinese README missing: $token";Need $en.Contains($token) "English README missing: $token"}
...
foreach($token in @('workflow_dispatch','package_v07.ps1','verify_v07_package.ps1','SHA256SUMS.txt','upload-artifact')){Need $release.Contains($token) "Release dry-run workflow missing: $token"}

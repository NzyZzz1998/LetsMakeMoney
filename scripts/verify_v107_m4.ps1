...
& $PythonExe (Join-Path $AppRoot "tests\verify_v107_m4.py")
if ($LASTEXITCODE -ne 0) {
    throw "v1.0.7 M4 static verification failed with exit code $LASTEXITCODE"
}
...
foreach ($Test in $Tests) {
    ...
    $Bundle = Join-Path $env:TEMP ("lmm-v107-m4-" + [IO.Path]::GetFileNameWithoutExtension($Test) + "-$PID.mjs")
    try {
        & $Esbuild $Source --bundle --platform=node --format=esm --outfile=$Bundle
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to bundle M4 behavior test: $Test"
        }
        & $Node $Bundle
        if ($LASTEXITCODE -ne 0) {
            throw "M4 behavior test failed: $Test"
        }
    }
    finally {
...
Write-Host "PASS LetsMakeMoney v1.0.7 M4 verification" -ForegroundColor Green

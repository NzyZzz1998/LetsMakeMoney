param(
    ...
    [string]$ExtractRoot = (Join-Path (Resolve-Path (Join-Path $PSScriptRoot "..")).Path ".tmp_release\verify_v04_package"),
    [string]$SmokeAppDataRoot = (Join-Path (Resolve-Path (Join-Path $PSScriptRoot "..")).Path ".tmp_appdata\verify_v04_package"),
    [int]$SmokeSeconds = 5
)
...
if ($manifest.package_name -ne "LetsMakeMoney-v0.4-beta-windows-x86_64") {
...
if ($manifest.version -ne "0.4-beta") {
...
try {
    ...
    if ($process.HasExited) {
        throw "LetsMakeMoney.exe exited during v0.4 package smoke test with code $($process.ExitCode)"
    }
...
Write-Host "v0.4 package smoke passed"

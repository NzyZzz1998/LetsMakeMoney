...
$buildScript = Get-Content -LiteralPath $build -Raw -Encoding UTF8
$sconstruct = Get-Content -LiteralPath (Join-Path $ProjectRoot "native\windows\SConstruct") -Raw -Encoding UTF8
...
if (-not $buildScript.Contains("use_mingw=yes")) {
    throw "MSYS2 native builds must explicitly select MinGW instead of relying on runner auto-detection."
}
foreach ($library in @("user32", "shell32")) {
    if (-not $sconstruct.Contains('"' + $library + '"')) {
        throw "Native SConstruct must explicitly link the Windows $library library."
    }
}

...
$BehaviorBundle = Join-Path $env:TEMP "lmm-v101-authoritative-sync-$PID.mjs"

try {
    & $Python (Join-Path $AppRoot "tests\verify_v101_m4.py")
    if ($LASTEXITCODE -ne 0) {
        throw "v1.0.1 M4 static verification failed."
    }
    ...
    & $Esbuild `
        (Join-Path $AppRoot "tests\authoritative-sync.behavior.ts") `
        --bundle `
        --platform=node `
        --format=esm `
        "--outfile=$BehaviorBundle"
    if ($LASTEXITCODE -ne 0) {
        throw "Authoritative sync behavior bundle failed."
    }
    & $Node $BehaviorBundle
    if ($LASTEXITCODE -ne 0) {
        throw "Authoritative sync behavior verification failed."
    }
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Native authoritative snapshot tests failed."
        }
    }
    finally {
        Pop-Location
    ...
    Write-Host "PASS LetsMakeMoney v1.0.1 M4 verification" -ForegroundColor Green
}
finally {

...
if (Test-Path -LiteralPath $currentPath) {
    ...
    if ($current -notmatch 'v0\.8 Beta' -or $current -notmatch 'v0\.8-beta' -or $current -notmatch 'GitHub Release') {
        Add-Finding "FAIL" "WRONG_STATUS" 'doc/current.md' "Current entry does not identify the published v0.8 tag and GitHub Release."
    }
}

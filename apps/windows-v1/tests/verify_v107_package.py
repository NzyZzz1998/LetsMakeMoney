...
# The drive-letter branch must not match the trailing `s:/` in `https://`.
...
def validate_v107_payload(package_path: Path, version: str, platform: str) -> None:
...
def main() -> int:
    ...
    try:
        ...
        validate_v107_payload(args.package, args.version, args.platform)
    except (
        base.IdentityError,
        OSError,
        UnicodeDecodeError,

...
def main() -> int:
    ...
    if "当前公开版本 | Windows v1.0.2 Stable" not in current:
        fail("doc/current.md 未声明当前公开版本为 v1.0.2 Stable")
    if "当前公开 tag | `v1.0.2`" not in current:
        fail("doc/current.md 未声明当前公开 tag 为 v1.0.2")
    ...
    print("v1.0/v1.0.1/v1.0.2 文档检查通过")
    print(f"- UTF-8 与乱码：{len(DOCUMENTS)} 份文档")
    print("- v1.0、v1.0.1 与 v1.0.2 必需事实源及本地链接完整")

...
The current release is **v0.8 Beta**. Source, portable Zip, licensing, automated regressions, and real Windows desktop Acceptance have passed. The unsigned test installer is not a public attachment. Use [doc/current.md](doc/current.md), [v0.8 verification](doc/releases/v0.8/verification.md), and [v0.8 release notes](doc/releases/v0.8/release-notes.md) as the current facts.

v0.8 adds calendar-month workday salary calculations, lunch-break deductions, single-weekend/double-weekend/alternating-week schedules, and safe v3-to-v4 configuration migration while preserving the v0.7 desktop-pet experience.
...
- **Installer:** v0.8 does not publish an installer. The existing test installer is not Authenticode-signed and is therefore not a Release attachment; use the portable Zip.
...
- Windows x86_64 only.
- The public v0.8 Beta release contains the portable Zip only. The test installer is unsigned and is not a Release attachment.
